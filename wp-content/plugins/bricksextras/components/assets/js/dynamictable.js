...
function xDynamicTable() {
    ...
    const extrasTable = function ( container ) {
    
    container.querySelectorAll('.brxe-xdynamictable').forEach((dynamicTable) => {

        const config = dynamicTable.getAttribute('data-x-table') ? JSON.parse(dynamicTable.getAttribute('data-x-table')) : {}

        let mygrid;

        let columnsArray = [];
        let columnsSaveArray = []; 
        let tableDataArray = [];

        let mobileVersion = true;

        const headerColumns = dynamicTable.querySelectorAll('.x-dynamic-table_table > thead > tr > th');
        const tableContainer = dynamicTable.querySelector('.x-dynamic-table_container')

        headerColumns.forEach(function(headerColumn) {

            let columnData = {
                name: headerColumn.innerHTML,
                formatter: (cell) => {
            ...
            if ( headerColumn.hasAttribute('data-x-column') ) {
                columnData.width = headerColumn.getAttribute('data-x-column')
                columnDataSave.width = headerColumn.getAttribute('data-x-column')
            }

                columnData.attributes = (cell) => {

                    if (cell) { 

                        if ( headerColumn.hasAttribute('data-x-column-attributes') ) {

                            let userAttributes = JSON.parse( headerColumn.getAttribute('data-x-column-attributes') );

                            if( typeof userAttributes.class !== "undefined" ){
                                    userAttributes.class += " gridjs-td";
                            }
                        ...
                        if ( headerColumn.hasAttribute('scope') ) {
                            return {
                                'scope': headerColumn.getAttribute('scope'),
                            }
                        }
                    }
                  ...
                  columnDataSave.attributes = (cell) => {
            ...
            if ( headerColumn.hasAttribute('data-x-type') && 'date' === headerColumn.getAttribute('data-x-type') ) {

                columnData.sort = {
                    compare: (a, b) => {

                        const cleaned_a = a.replace(/(\d+)(st|nd|rd|th)/, '$1');
                        const cleaned_b = b.replace(/(\d+)(st|nd|rd|th)/, '$1');
            ...
            } else if ( headerColumn.hasAttribute('data-x-type') && 'prevent_sortable' === headerColumn.getAttribute('data-x-type') ) {
                columnData.sort = false
            } else if ( 'true' === config.sort && headerColumn.hasAttribute('data-x-type') && 'string' === headerColumn.getAttribute('data-x-type') ) {
                
               /* Default case-insensitive string sorting */
                columnData.sort = {
                    compare: (a, b) => {
                        const str_a = String(a).toLowerCase();
                        const str_b = String(b).toLowerCase();
            ...
            columnsSaveArray.push( columnDataSave )
        })

        if (window.xTable){ 
            window.xTable.Columns[dynamicTable.dataset.xId] = columnsSaveArray;
        }

        const tableRows = dynamicTable.querySelectorAll('.x-dynamic-table_table > tbody > tr')
        tableRows.forEach((tableRow,index) => {

            let dataArray = [];

            const tableRowDatas = tableRow.querySelectorAll('td[data-x-type]');

            tableRowDatas.forEach((tableRowData) => {

                if ( 'number' === tableRowData.getAttribute('data-x-type') ) {
                    if ( isNaN( parseFloat( tableRowData.innerHTML ) ) ) {
                        dataArray.push( '' )
                    } else {
                        dataArray.push( parseFloat( tableRowData.innerHTML ) )
                    }
                    
                } else {
                    dataArray.push( tableRowData.innerHTML )
                }
        ...
        if (window.xTable){ 
            window.xTable.Data[dynamicTable.dataset.xId] = tableDataArray;
        }
        


        const gridConfig = {
            //from: dynamicTable.querySelector(".x-dynamic-table_table"),
            columns: columnsArray,
            data: tableDataArray,
            // enable search
            search: {
                enabled: 'true' === config.search,
            },
            language: {
                'search': {
                ...
                'noRecordsFound': config.noRecordsFound
              },
            // enable pagination
            pagination: {
                enabled: null != config.rowsPerPage,
                limit: config.rowsPerPage,
                buttonsCount: config.buttonsCount,
                summary: 'true' === config.summary,
            ...
            resizable: 'true' === config.resizable,
            sort: 'true' === config.sort,
        }

        /* check if already there, if so just update */
        if ( dynamicTable.querySelector(".x-dynamic-table_container") ) {

            if ( dynamicTable.querySelector(".x-dynamic-table_container").innerHTML === '' ) {
                let mygrid = new gridjs.Grid(gridConfig).render(dynamicTable.querySelector(".x-dynamic-table_container"));

                if (window.xTable){ 
                    window.xTable.Instances[dynamicTable.dataset.xId] = mygrid;
                    
                    // Dispatch custom event to notify that table instance is ready
                    dynamicTable.dispatchEvent(new CustomEvent('x_table:init'));
                }
                ...
                headerColumns.forEach((headerColumn, index) => {
                    const sortOrder = headerColumn.getAttribute('data-x-initial-sort');
                ...
                if (initialSortColumn !== null && initialSortDirection) {
                        const currentData = mygrid.config.data;
                        const currentColumns = mygrid.config.columns;
                        const columnConfig = currentColumns[initialSortColumn];
                        
                        const sortedData = [...currentData].sort((a, b) => {
                            let result = 0;
                            if (columnConfig?.sort && typeof columnConfig.sort.compare === 'function') {
                                result = columnConfig.sort.compare(a[initialSortColumn], b[initialSortColumn]);
                            } else {
                                const valueA = a[initialSortColumn];
                                const valueB = b[initialSortColumn];
                            ...
                            return initialSortDirection === 'desc' ? -result : result;
                        });
                ...
                if (mygrid != undefined) {
                    mygrid.updateConfig(gridConfig).forceRender();
                }
            }

            /* add labels for mobile table */
            if ( dynamicTable.querySelector(".x-dynamic-table_table") ) {

                let columnHeadings = []

                dynamicTable.querySelectorAll(".x-dynamic-table_table th.label").forEach((columnHeading) => {
                    columnHeadings.push( columnHeading.innerHTML )
                })
                ...
                observer.observe(dynamicTable.querySelector('.gridjs-tbody'), options)
                ...
                function doMobileTable() {
                    dynamicTable.querySelectorAll(".gridjs-tr").forEach((column) => {
                        column.querySelectorAll(".gridjs-td").forEach((row,index) => {
        ...
        if ( ! document.querySelector('body > .brx-body.iframe') ) {
            if ( dynamicTable.querySelector(".x-dynamic-table_table") ) {
                dynamicTable.querySelector(".x-dynamic-table_table").remove()
            }
        }

        function maybeStack() {
            if (window.innerWidth < config.stack) {
                tableContainer.classList.add('x-dynamic-table_stacked')
            } else {
                tableContainer.classList.remove('x-dynamic-table_stacked')
            }
        }
...
setTimeout(() => {
    extrasTable(document);
}, 30)

const xDynamicTableAJAX = xExtrasRegisterAJAXHandler('doExtrasTable', true);

window.doExtrasTable = extrasTable;


if (typeof bricksextras !== 'undefined') {

    bricksextras.dynamictable = {
      refresh: (brxParam) => {
        let target = brxParam?.target || false
        if ( target ) {
          xDynamicTableRefresh(target.id)
        }
      },
      export: (brxParam, title = false) => {
        let target = brxParam?.target || false
        if ( target ) { 
          xDynamicTableExportToCSV(target, title)
        }
      }
     }
}
...
function xDynamicTableRefresh(elementID) {

    const element = document.getElementById(elementID)
    const identifier = element.getAttribute('data-x-id')

    const instance = xTable.Instances[identifier]
    const columns = xTable.Columns[identifier]
    const data = xTable.Data[identifier]

    if (!instance || !columns || !data) {
        return
    }
    ...
    setTimeout(() => {
        instance.updateConfig(newconfig).forceRender();
    }, 10)

}

function xDynamicTableExportToCSV(element, title = false) {

    const identifier = element.getAttribute('data-x-id')
    const grid = xTable.Instances[identifier]

    // Retrieve headers from Grid.js configuration
  const headers = grid.config.columns.map(column => column.name || column).join(",");

  // Retrieve data rows from Grid.js configuration
  const rows = grid.config.data.map(row => {
    return row.map(cell => {
      // Convert cell to string and escape double quotes
      let cellStr = typeof cell === 'string' ? cell.replace(/"/g, '""') : cell;
  ...
  // Combine headers and rows without any extra titles
  ...
  a.href = url;
  a.download = title ? title + ".csv" : "table.csv"; 
...
document.addEventListener("DOMContentLoaded",function(e){
    bricksIsFrontend&&xDynamicTable()
});
